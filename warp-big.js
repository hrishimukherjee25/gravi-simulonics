...
function warpField(x, y, t) {
    ...
    // Make the bubble larger by adjusting the decay and scaling factors
    const contraction = 2 * Math.exp(-r * r / 2) * Math.cos(2 * Math.PI * t); // Larger radius
    const expansion = 2 * Math.exp(-r * r / 2) * Math.sin(2 * Math.PI * t);
