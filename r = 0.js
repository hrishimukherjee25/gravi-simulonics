...
function singularityField(x, y, t) {
    const r = Math.sqrt(x * x + y * y) || 1e-5; // Avoid division by zero, substitute small value for r ~ 0

    // Diverging curvature
    const curvature = Math.min(100, 1 / Math.pow(r, 2)); // Cap divergence for visual clarity

    // Quantum fluctuations (oscillations near r = 0)
    const quantumEffect = Math.sin(10 * Math.PI * r + t) * Math.exp(-r * 5);

    // Combine effects
    const contraction = -curvature + quantumEffect;
    const expansion = curvature + quantumEffect;
...
function updateVectorField(t) {
    ...
    for (let i = 0; i < origins.length / 3; i++) {
        ...
        const { contraction, expansion } = singularityField(x, y, t);
