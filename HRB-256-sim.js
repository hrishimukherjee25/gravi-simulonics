...
function init() {
    ...
    camera.position.z = 50; // Farther back for better view
...
function simulonField(x, y, t) {
    const r = Math.sqrt(x * x + y * y) || 1; // Avoid division by zero

    // Dimensional Synthesis (Energy flux)
    const energyFlux = Math.exp(-r * r / 5) * Math.sin(2 * Math.PI * t);

    // Numerical Hierarchies (Fractal perturbations)
    const fractalPerturbation = Math.sin(r * Math.PI) * Math.cos(t * 2 * Math.PI);

    // Simulation Energy Dynamics (Harmonic fields)
    const harmonicField = Math.cos(2 * Math.PI * t) * Math.sin(r);

    // Combined effect (Symbolic expansion)
    const contraction = energyFlux * harmonicField + fractalPerturbation;
    const expansion = energyFlux * harmonicField - fractalPerturbation;
...
function updateVectorField(t) {
    ...
    for (let i = 0; i < origins.length / 3; i++) {
        ...
        const { contraction, expansion } = simulonField(x, y, t);
