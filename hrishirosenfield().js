...
function hrishiRosenField(x, y, t) {
    const r = Math.sqrt(x * x + y * y) || 1; // Avoid division by zero
    const rs = 2; // Schwarzschild radius

    // Modified Schwarzschild metric with flux dynamics
    const curvature = Math.sqrt(1 - rs / r);
    const flux = Math.sin(2 * Math.PI * t) * Math.exp(-r / 5);

    // Symbolic multiversal flux (\(\Psi_{\text{HRP}}\))
    const multiversalFlux = Math.sin(r * Math.PI) * Math.cos(t);

    // Combine effects
    const contraction = curvature * flux;
    const expansion = flux * multiversalFlux;
...
function updateVectorField(t) {
    ...
    for (let i = 0; i < origins.length / 3; i++) {
        ...
        const { contraction, expansion } = hrishiRosenField(x, y, t);
